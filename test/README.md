...
| `test/e2e/` | Real-browser checks (need Chromium via Playwright, not run by `node --test`): `node test/e2e/smoke.js` (14 functional checks; set `SERVE_ROOT` to a built `_site` to test the CI bundle) and `node test/e2e/a11y.js` (11 accessibility checks) |
